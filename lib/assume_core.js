...
function createReturnValueStub(collaborator, funcName, args, returnValue, trackInfo) {
  ...
  collaborator[funcName].mapArgs2ReturnValueStub[args] = escapeUndefined(returnValue);
}

function createThrowErrorStub(collaborator, funcName, args, errorMessage, trackInfo) {
  args = stringify(args);
  stub(undefined, collaborator, funcName, trackInfo);
  collaborator[funcName].mapArgs2ThrowErrorStub[args] = escapeUndefined(errorMessage);
}

function createCallbackStub(collaborator, funcName, args, callbackArgs, callbackIndex, trackInfo) {
  args = stringify(args);
  stub(undefined, collaborator, funcName, trackInfo);
  collaborator[funcName].mapArgs2CallbackArgsStub[args] = {};
  collaborator[funcName].mapArgs2CallbackArgsStub[args].callbackIndex = callbackIndex;
  collaborator[funcName].mapArgs2CallbackArgsStub[args].args = callbackArgs;
}
...
function stub(repo, collaborator, funcName, trackInfoOfAssumption) {
  ...
  function stubbedFunc() {
    ...
    function isCallbackStubDefined() {
      return collaborator[funcName].mapArgs2CallbackArgsStub[actualStringifiedArgs] !== undefined;
    }

    function isReturnStubDefined() {
      return collaborator[funcName].mapArgs2ReturnValueStub[actualStringifiedArgs] !== undefined;
    }

    function isThrowErrorStubDefined() {
      return collaborator[funcName].mapArgs2ThrowErrorStub[actualStringifiedArgs] !== undefined;
    }
    ...
    function handleStubCall() {
      ...
      if (isThrowErrorStubDefined()) {
        var errorMessage = collaborator[funcName].mapArgs2ThrowErrorStub[actualStringifiedArgs];
        throw new Error(errorMessage);
      }
      if (isCallbackStubDefined()) {
        var callbackData = collaborator[funcName].mapArgs2CallbackArgsStub[actualStringifiedArgs];
        var callbackArgs = callbackData.args;
        var callbackIndex = callbackData.callbackIndex;
        var cb = actualArgs[callbackIndex];
        assertIsFunction(cb, callbackIndex);
      ...
      if (isReturnStubDefined()) {
         returnValue = unescapeUndefined(collaborator[funcName].mapArgs2ReturnValueStub[actualStringifiedArgs]);
      }
      return returnValue;
    }
  ...
  stubbedFunc.mapArgs2ReturnValueStub = {};
  stubbedFunc.mapArgs2CallbackArgsStub = {};
  stubbedFunc.mapArgs2ThrowErrorStub = {};
