...
var parseActionString = require('./actionString').parse;
var fs = require('fs');
...
function read(file) {
  ...
  var array = transform(json, ["name", "func", "args", "action", "type", "file", "line", "callermethod"], {"args": parseArgsString, "action": parseActionString});
  return sort(array);
}
  ...
  verifications.forEach(function (verification) {
    ...
    assumptions.forEach(function (assumption) {
      if (traversedElements.indexOf(assumption) == -1) {
        ...
        var subpaths = findPaths(array, assumption, [].concat(traversedElements));
        if (subpaths.length > 0) {
          subpaths.forEach(function (subpath) {
            paths.push([assumption].concat(subpath));
          });
        } else {
          paths.push([assumption]);
        }
      }
    });
  });
