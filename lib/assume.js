...
function assume(repo, testdoubleRepo, lib) {
  ...
  function andReturn(returnValue) {
    ...
    saveAssumption(createReturnValueString(returnValue));
  }
  ...
  function andCallsCallbackWith(nCallbackIndex) {
    ...
    saveAssumption(createCallbackString(1, argumentsWithWhichCallbackShouldBeCalled));
    return {
      andReturn : andReturn
    }
  }
    ...
    if (!lib[funcName]) {
      lib[funcName] = function () {
        ...
        if (isCallbackDefined(stringifiedArgs)) {
          ...
          var storedObj = 
          save([libName, funcName, stringifiedArgs, createCallbackString(1, callbackArgs), 
        ...
        if (isReturnDefined(stringifiedArgs)) {
          ...
          var storedObj = 
            save([libName, funcName, stringifiedArgs, createReturnValueString(returnValue), 
              "calledBy", trackInfo.file, trackInfo.line, trackInfo.func])
            .to(repo);
  ...
  function createReturnValueString(returnValue) {
    return "r:" + stringify(returnValue);
  }
  
  function createCallbackString(index, args) {
    return "cb:" + index + "->" + stringify(args);
  }
