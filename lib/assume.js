...
function assume(repo, lib) {
  ...
  function withArgs() {
    ...
    stringifiedArgs = stringify(args);
    return {
      andReturns : andReturns,
  ...
  function andReturns(returnValue) {
    lib[funcName].mapArgs2ReturnValue[stringifiedArgs] = escapeUndefined(returnValue);
  ...
  function andCallsCallbackWith() {
    ...
    lib[funcName].mapArgs2CallbackArgs[stringifiedArgs] = argumentsWithWhichCallbackShouldBeCalled;
  ...
  function andThrowsError(message) {
    lib[funcName].mapArgs2ThrowError[stringifiedArgs] = escapeUndefined(message);
  ...
  function saveAssumption(actionString) {
    var storedObj = save([libName, funcName, stringifiedArgs, actionString, "assume", trackInfo.file, trackInfo.line, trackInfo.func]).to(repo);
  }
