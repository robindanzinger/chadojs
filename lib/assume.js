...
function assume(repo, lib) {
  ...
  function andCallsCallbackWith(nCallbackIndex) {
    ...
    lib[funcName].mapArgs2CallbackArgs[args] = escapeUndefined(argumentsWithWhichCallbackShouldBeCalled);
    ...
    if (!lib[funcName]) {
      lib[funcName] = function () {
        ...
        if (isCallbackDefined(stringifiedArgs)) {
          returnValue = null;
          var callbackArgs = unescapeUndefined(lib[funcName].mapArgs2CallbackArgs[stringifiedArgs]);
