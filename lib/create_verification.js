...
function getReturnValueAsString(action) {
  return action.value && action.type === 'returnValue' 
    ? convertToCodeString(action.value) : undefined; 
}

function getCallbackArgsAsCommaSeparatedString(action) {
  return action.value && action.type === 'callback' 
    ? convertArgsArrayToCommaSeparatedCodeString(action.value) : undefined;
}
