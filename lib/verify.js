...
function verify(repo, doubleName) {
  ...
  function onReturn(sut) {
    ...
    if (!is(expectedReturnValue).similarTo(actualReturnValue)) 
    {
      throw new Error('VerificationError:' 
          + '\n  Expected return value: ' + stringifyErrorArgument(expectedReturnValue) 
          + '\n  Actual return value  : ' + stringifyErrorArgument(actualReturnValue)); 
    }
  }
  ...
  function onError(sut) {
    ...
    try {
      ...
      if (error.message !== expectedErrorMessage) {
        throw new Error('VerificationError:'
          + '\n Expected error message: ' + stringifyErrorArgument(expectedErrorMessage) 
          + '\n Actual error message  : ' + stringifyErrorArgument(error.message));
      }
      return;
    }
  ...
  function callbackFunction() {
    ...
    {
      throw new Error('VerificationError:' 
          + '\n  Expected callback arguments: ' + stringifyErrorArgument(expectedCallbackArgs) 
          + '\n  Actual callback arguments: ' + stringifyErrorArgument(actualCallbackArgs)); 
    }
    verifyCallback();
  }
